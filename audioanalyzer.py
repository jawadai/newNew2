...
class AudioAnalyzer:
    ...
    #                     zip_ref.extractall("/tmp/silero-vad")
                
    #             import sys
    #             sys.path.append("/tmp/silero-vad/silero-vad-master")
    #             from utils_vad import get_speech_timestamps, read_audio, save_audio, collect_chunks
                
    #             model = torch.jit.load("https://models.silero.ai/vad_models/silero_vad.jit")
    #             utils = (get_speech_timestamps, read_audio, save_audio, collect_chunks)
        ...
        try:
            ...
            def get_speech_timestamps(audio, model, threshold=0.5, sampling_rate=16000, min_speech_duration_ms=250, min_silence_duration_ms=100):
                ...
                with torch.no_grad():
                    ...
                    window_size_samples = sampling_rate
                    speech_probs = []
                    ...
                    for i in range(0, len(audio), window_size_samples):
                        ...
                        probs = model(chunk, sampling_rate).squeeze()
                        speech_probs.extend(probs.tolist())
                    ...
                    mask_shift[1:] = mask[:-1]
                    starts = torch.nonzero(mask & ~mask_shift).squeeze()
                    ends = torch.nonzero(~mask & mask_shift).squeeze()
                    ...
                    min_speech_samples = min_speech_duration_ms * sampling_rate // 1000
                    min_silence_samples = min_silence_duration_ms * sampling_rate // 1000
                    
                    segments = []
                    for start, end in zip(starts.tolist(), ends.tolist()):
                        if end - start >= min_speech_samples:
                            segments.append({
                                'start': start * window_size_samples // len(speech_probs),
                                'end': end * window_size_samples // len(speech_probs)
                            })
                    
                    return segments
            
            utils = (get_speech_timestamps, None, None, None)
            model = model.to(self.config.device)
            return model, utils
        except Exception as e:
            self.logger.error(f"Failed to load Silero VAD model: {str(e)}")
